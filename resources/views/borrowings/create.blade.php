            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('borrowings.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" id="borrowed_at" name="borrowed_at" value="{{ old('borrowed_at', date('Y-m-d')) }}" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1 @error('borrowed_at') border-red-500 @enderror" required>
                        @error('borrowed_at')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
